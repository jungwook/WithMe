...
@protocol FloatingDrawerAnimation <NSObject>
 ...
 */
- (void)presentationWithSide:(FloatingDrawerSide)drawerSide sideView:(UIView *)sideView
 ...
 *  @param completion A completion block to be called when the presentation is finished
 */
- (void)dismissWithSide:(FloatingDrawerSide)drawerSide sideView:(UIView *)sideView
             centerView:(UIView *)centerView animated:(BOOL)animated completion:(void(^)(BOOL finished))completion;
