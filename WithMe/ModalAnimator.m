...
- (void)animateTransitionCopy:(id <UIViewControllerContextTransitioning>)transitionContext
{
    CGFloat toOffset = 0, fromOffset = 0;
    ...
    if ([fromVC respondsToSelector:@selector(offsetHeight)]) {
        fromOffset = toVC.offsetHeight;
    }
    
    CGRect finalFrame = CGRectMake(0,
                                   kOffsetFromTop,
                                   CGRectGetWidth(toVC.view.bounds),
                                   CGRectGetHeight(toVC.view.bounds)-kOffsetFromTop);
    ...
    if (self.presenting)
    {
        [container insertSubview:[[UIApplication sharedApplication].keyWindow snapshotViewAfterScreenUpdates:NO] atIndex:0];
        fromVC.view.alpha = 0;
        fromVC.view.hidden = YES;
        ...
        [UIView animateWithDuration:[self transitionDuration:transitionContext]*2.5
                         ...
                         animations:^{
                             backgroundView.alpha = _backgroundShadeAlpha;
                             [(UIView*)[container.subviews objectAtIndex:0] setTransform:_scaleTransform];
                         } completion:nil];
