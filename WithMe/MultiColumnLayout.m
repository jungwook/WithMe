...
NSString *const MultiColumnElementKindSectionHeader = @"MultiColumnElementKindSectionHeader";
NSString *const MultiColumnElementKindSectionFooter = @"MultiColumnElementKindSectionFooter";

@interface MultiColumnLayout ()
/// The delegate will point to collection view's delegate automatically.
@property (nonatomic, weak) id <MultiColumnDelegateLayout> delegate;
...
@implementation MultiColumnLayout

/// How many items to be union into a single rectangle
static const NSInteger unionSize = 20;

static CGFloat MultiColumnFloorCGFloat(CGFloat value) {
...
- (void)setItemRenderDirection:(MultiColumnLayoutItemRenderDirection)itemRenderDirection {
...
- (CGFloat)itemWidthInSectionAtIndex:(NSInteger)section {
  ...
  return MultiColumnFloorCGFloat((width - (columnCount - 1) * columnSpacing) / columnCount);
}
...
- (id <MultiColumnDelegateLayout> )delegate {
  return (id <MultiColumnDelegateLayout> )self.collectionView.delegate;
}
...
- (void)commonInit {
  ...
  _itemRenderDirection = MultiColumnLayoutItemRenderDirectionShortestFirst;
}
...
- (void)prepareLayout {
  ...
  NSAssert([self.delegate conformsToProtocol:@protocol(MultiColumnDelegateLayout)], @"UICollectionView's delegate should conform to MultiColumnDelegateLayout protocol");
  ...
  for (NSInteger section = 0; section < numberOfSections; ++section) {
    ...
    CGFloat width = self.collectionView.bounds.size.width - sectionInset.left - sectionInset.right;
    NSInteger columnCount = [self columnCountForSection:section];
    CGFloat itemWidth = MultiColumnFloorCGFloat((width - (columnCount - 1) * columnSpacing) / columnCount);
    ...
    if (headerHeight > 0) {
      attributes = [UICollectionViewLayoutAttributes layoutAttributesForSupplementaryViewOfKind:MultiColumnElementKindSectionHeader withIndexPath:[NSIndexPath indexPathForItem:0 inSection:section]];
    ...
    for (idx = 0; idx < itemCount; idx++) {
      ...
      if (itemSize.height > 0 && itemSize.width > 0) {
        itemHeight = MultiColumnFloorCGFloat(itemSize.height * itemWidth / itemSize.width);
      }
    ...
    if (footerHeight > 0) {
      attributes = [UICollectionViewLayoutAttributes layoutAttributesForSupplementaryViewOfKind:MultiColumnElementKindSectionFooter withIndexPath:[NSIndexPath indexPathForItem:0 inSection:section]];
...
- (UICollectionViewLayoutAttributes *)layoutAttributesForSupplementaryViewOfKind:(NSString *)kind atIndexPath:(NSIndexPath *)indexPath {
  ...
  if ([kind isEqualToString:MultiColumnElementKindSectionHeader]) {
    attribute = self.headersAttribute[@(indexPath.section)];
  } else if ([kind isEqualToString:MultiColumnElementKindSectionFooter]) {
...
- (NSUInteger)nextColumnIndexForItem:(NSInteger)item inSection:(NSInteger)section {
  ...
  switch (self.itemRenderDirection) {
    case MultiColumnLayoutItemRenderDirectionShortestFirst:
      index = [self shortestColumnIndexInSection:section];
      break;

    case MultiColumnLayoutItemRenderDirectionLeftToRight:
      index = (item % columnCount);
      break;

    case MultiColumnLayoutItemRenderDirectionRightToLeft:
